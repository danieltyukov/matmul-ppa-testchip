...
//
// Candidate 3: sign-magnitude encoded datapath.
//
// Operands are converted from two's complement to sign-magnitude inside the
// engine, multiplied as unsigned magnitudes, and the product sign is applied
// once at the array output. The conversion sits inside the engine rather than in
// shared logic so that all candidates keep one identical interface and so that
// the measured cost of the encoding is charged to the candidate that uses it.
//
// This is the candidate that tests whether lower operand switching activity
// translates into a real power advantage. See docs/PPA_METHODOLOGY.md.
//
